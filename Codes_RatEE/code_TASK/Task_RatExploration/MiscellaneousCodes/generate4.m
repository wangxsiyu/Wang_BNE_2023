function game = generate4(Nguided, Nfrees, T, rlist, rlistfreq, hbs, R, folder, rndsd, dt, ratname)
    ...
    third(1:2:(2*N-1),1) = g1(3).x_cb;
    third(2:2:(2*N),1) = g2(3).x_cb;
    thirdr(1:2:(2*N-1),1) = g1(4).x_cb;
    thirdr(2:2:(2*N),1) = g2(4).x_cb;
...
%         end
    ...
    game = [hb', fd, Rexploit', Rexplore', third, thirdr, nFree];
    ng = size(game,1);
...
%     halfNguided = round(Nguided/2);
    for i = 1:ng
        ...
        for hi = 1:nF
            ...
            if tr(1) == 0
                if hi == 1 && t3 > 0
                    fprintf(fileID,'%d%d%d\t%d\t1\t0\t%d%d%d\n', tfd(2), tfd(1), t3, T, tr(2), tr(1), t3r);
                else
                    fprintf(fileID,'%d%d\t%d\t1\t0\t%d%d\n', tfd(2), tfd(1), T, tr(2), tr(1));
                end
            else
                if hi == 1 && t3 > 0
                    fprintf(fileID,'%d%d%d\t%d\t1\t0\t%d%d%d\n', tfd(1), tfd(2), t3, T, tr(1), tr(2), t3r);
                else
    ...
    disp(str);
    disp(sprintf('total runs = %d', ng*(Nguided + mean(Nfrees))*2));
end
