function game = generate3(Nguided, Nfree, T, rlist, rlistfreq, hbs, R, folder, rndsd, dt, ratname)
    rand('seed', rndsd +sum(clock()) );
    if ~exist('folder')
        folder = './';
    end
    outputfile = strcat(ratname, '_', '3lightV1_',num2str(dt));
...
%         end
    ...
    game = [hb', fd, Rexploit', Rexplore', third, thirdr];
    ng = size(game,1);
