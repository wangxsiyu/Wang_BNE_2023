function game = formatRat_game(game, maze)
    ...
    fds = cellfun(@(x)W.extend_nan(arrayfun(@(t)letter2num(t, maze), x), nfdmax), game.feeders, 'UniformOutput', false);
    fds = vertcat(fds{:});
    ...
    drops = game.drops;
    dps = cellfun(@(x) W.extend_nan(arrayfun(@(t)str2num(t), x), nfdmax), drops, 'UniformOutput', false);
    dps = vertcat(dps{:});
...
function out = getfd(x, id)
    ...
    x = x .* id;
    x(x == 0) = NaN;
    x = W.unique_col(x', 0,'stable');
    x = W.cellfun(@(t)t', x);
    x = W.cell_extend_nan(x);
    x = vertcat(x{:});
    x(x == x0) = 0;
    out = floor(x);
end
