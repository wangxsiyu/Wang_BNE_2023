function figureS1_split_awaytowards(plt, rat, human)
...
plt.setfig([1:4 8:11],'ylabel', {{'human';'(switch away)'},'','','',...
    {'human';'p(switch back)'},'','',''}, ...
    ...
    'legloc', {'NorthEast','NorthEast','NorthEast','NorthEast','NorthEast','NorthEast','NorthEast','NorthEast'}, ...
    'title', {'H = 1', 'H = 2', 'H = 5', 'H = 10','H = 1', 'H = 2', 'H = 5', 'H = 10'}, ...
    'xtick',{1, 1:2, 2:2:6, 5:5:15, 1, 1:2, 2:2:6, 5:5:15},'xticklabel',{{},{1:2},{},{},{},{1:2},{},{}});
plt.setfig([5:7 12:14], 'ylabel', {{'rat';'p(switch away)'},'','',...
    {'rat';'p(switch back)'},'',''}, ...
...
tav = ff(human.gp.av_gp_av_cc_sw_unguided);
tse = ff(human.gp.ste_gp_av_cc_sw_unguided);
for i = 1:4
    rid = [((5-i) * 2 -1),(5-i) * 2];
    plt.new;
    if i == 4
    ...
    if i == 4 
        ...
        lgd.Position = [0.6519 0.6911 0.3681 0.2525];
        lgd.FontSize = plt.param_figsetting.fontsize_leg;
    end
end
tav = ff(rat.gp.av_gp_av_cc_sw_unguided);
tse = ff(rat.gp.ste_gp_av_cc_sw_unguided);
for i = 1:3
    rid = [((4-i) * 2 -1),(4-i) * 2];
    plt.new;
    if i == 3
    ...
    if i == 3
        [lgd] = columnlegend(4,  {" "," "," "," "," "," ",'guided = bad', 'guided = good'}');
        lgd.Position = [0.6519 0.4911 0.3681 0.2525];
        lgd.FontSize = plt.param_figsetting.fontsize_leg;
    end
end
tav = ff(human.gp.av_gp_av_cc_sw_guided);
tse = ff(human.gp.ste_gp_av_cc_sw_guided);
for i = 1:4
    rid = [((5-i) * 2 -1),(5-i) * 2];
    plt.new;
    if i == 4
...
tav = ff(rat.gp.av_gp_av_cc_sw_guided);
tse = ff(rat.gp.ste_gp_av_cc_sw_guided);
for i = 1:3
    rid = [((4-i) * 2 -1),(4-i) * 2];
    plt.new;
    if i == 3
