...
width = numcolumns*pos(3)*0.5;
newheight = (pos(4)/numlines)*numpercolumn/2;
rescale = pos(3)/width * 0.5;
...
switch type,
    ...
    case {'hggroup'}
        ...
        elseif strcmpi(subtype, 'hggroup'), % errorbar
            ...
            line_width = (xdata(2)-xdata(1))*rescale;       % rescaled linewidth to match original
            spacer = (xdata(1)*rescale)/2;                      % rescaled spacer used for margins
        end
end
...
%                 v = v* 0.5;
                ...
                xx = [col/numcolumns/2+spacer col/numcolumns/2+spacer+line_width*2];
                set(child(2), 'xdata', xx);
                
%                 set(object_h(linenum+1), 'ydata', [height-(position-1)*sheight height-(position-1)*sheight]);
%                 set(object_h(linenum+1), 'xdata', [col/numcolumns+spacer*3.5 col/numcolumns+spacer*3.5]);
                
                set(object_h(labelnum), 'position', [col/numcolumns/2+spacer*2+line_width + 0.08 height-(position-1)*sheight]);
            end
    end
