function [feeders, rewards] = generate_horizon_v4(N, cond_horizon, steplevel, levels, folder)
%   N - number of trials
...
rand('seed', sum(clock()));
outputfile = strcat('taskset_horizonv4_h', num2str(cond_horizon), '_', datestr(now,30),'.txt');
...
fclose(fileID);
disp('file generation complete - task horizon v4 - rewarded homebase');
end
