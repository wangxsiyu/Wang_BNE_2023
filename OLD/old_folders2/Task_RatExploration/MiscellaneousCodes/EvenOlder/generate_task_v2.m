function generate_task_v2(Nrep, Nguided, Nfree, T, dps, hb, folder)
    if ~exist('folder')
        folder = './';
    end
    rand('seed', sum(clock()));
    outputfile = strcat('taskset_drop_', datestr(now,30));
    fileID = fopen(fullfile(folder, [outputfile, '.txt']),'w');
    fprintf(fileID, '1\t1\t0\n');
    rlist = getrlist(dps);
    rlist = repmat(rlist, Nrep,1);
    nr = size(rlist, 1);
    rlist = rlist(randperm(nr),:);
    hblist = repmat(reshape(hb,[],1), nr, 1);
    hblist = hblist(1:nr);
    tod = [ones(1,Nguided),zeros(1,Nguided)]+1;
    for i = 1:nr
        odlist(i,:) = tod(randperm(length(tod)));
    end
    for i = 1:nr
        thb = hblist(i);
        tfd = findbandit(thb);
